/**
 * Script para sincronizar modelos de todas las marcas de motos desde la API externa
 * Ejecutar con: node sync-motorcycle-models.js
 */
...
// URL base de la API externa para motos
...
async function fetchModelsFromAPI(brandSlug, retries = 0) {
  try {
    const url = `${EXTERNAL_API_BASE}/marques-moto?marca=${brandSlug}`;
    console.log(`📡 Calling API: ${url}`);
    
    const response = await axios.get(url, {
      timeout: CONFIG.timeout,
      headers: {
        'User-Agent': 'Kars.ad Motorcycle Model Sync',
...
async function syncAllMotorcycleModels() {
  try {
    console.log('🏍️ Iniciando sincronización de modelos de motos...');
    
    // Obtener todas las marcas de motos de la base de datos
    ...
    console.log(`📗 Encontradas ${motorcycleBrands.length} marcas de motos para sincronizar`);
    ...
    for (let i = 0; i < motorcycleBrands.length; i += CONFIG.batchSize) {
      const batch = motorcycleBrands.slice(i, i + CONFIG.batchSize);
      
      console.log(`\n📦 Procesando lote ${Math.floor(i / CONFIG.batchSize) + 1}/${Math.ceil(motorcycleBrands.length / CONFIG.batchSize)} (${batch.length} marcas)`);
      ...
      if (i + CONFIG.batchSize < motorcycleBrands.length) {
    ...
    console.log('✅ Sincronización de motos completada exitosamente');
...
async function syncSingleMotorcycleBrand(brandSlug) {
  try {
    console.log(`🏍️ Sincronizando marca específica: ${brandSlug}`);
    ...
    if (brand.vehicleType !== 'motorcycle') {
      console.error(`❌ La marca ${brand.name} no es de tipo 'motorcycle'`);
      process.exit(1);
    }
...
if (require.main === module) {
  ...
  if (args.length > 0) {
    ...
    syncSingleMotorcycleBrand(brandSlug)
      .then(() => {
...
module.exports = { syncAllMotorcycleModels, syncSingleMotorcycleBrand };